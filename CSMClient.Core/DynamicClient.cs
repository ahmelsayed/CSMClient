    ...
    public class DynamicClient : DynamicObject
    {
        ...
        private bool TryHandleDynamicCall(string name, out object result)
        {
            ...
            result = string.IsNullOrEmpty(this._authorizationHeader)
                ? new DynamicClient(this._apiVersion, this._authHelper, this._azureEnvironment, url)
                : new DynamicClient(this._apiVersion, this._authorizationHeader, url);
